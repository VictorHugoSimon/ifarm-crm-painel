...
const totp = {
  ...
  otpauthURL(seg, conta, emissor){ const em=emissor||'Terra Pulse'; return 'otpauth://totp/'+encodeURIComponent(em+':'+conta)+'?secret='+seg+'&issuer='+encodeURIComponent(em)+'&algorithm=SHA1&digits=6&period=30'; },
...
/* Equipe interna Terra Pulse — semeada com senha inicial 12345678 (precisaTrocar=true). */
const PADRAO = [
  ...
  try {
    ...
    if (event.httpMethod === 'POST') {
      ...
      if (twoFAObrigatorio()) {
        ...
        const otpauth = totp.otpauthURL(seg, u.email, 'Terra Pulse');
