...
async function enviarBoasVindas(email, nome, perfil){
  ...
  if (!key) return false;
  const from = process.env.RESEND_FROM || process.env.MAIL_FROM || 'iFarm CRM <nao-responda@ifarm.agr>';
  const replyTo = process.env.RESEND_REPLY_TO || 'suporte@ifarm.agr';
